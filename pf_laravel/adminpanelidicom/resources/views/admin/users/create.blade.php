<x-admin-fluid>
    <x-slot name="title">
        <h2 class="mb-0">{{ __('Create New User') }}</h2>
    ...
    <x-form :action="route('admin.users.store')" method="post">
        <div class="form-floating mb-3">
            <input name="name" value="{{ old('name') }}" class="form-control" id="floatingInput"
                   placeholder="{{ __('Name') }}">
            <label for="floatingInput">{{ __('Name') }}</label>
        </div>
        <div class="form-floating mb-3">
            <input name="email" value="{{ old('email') }}" class="form-control" id="floatingInput"
                   placeholder="{{ __('Email') }}">
            <label for="floatingInput">{{ __('Email') }}</label>
        </div>
        <div class="form-floating mb-3">
            <input name="email_verified_at" value="{{ old('email_verified_at') }}"
                ...
                @foreach($roles as $role)
                    <div class="form-check">
                        <input name="roles[]" value="{{ $role->name }}" class="form-check-input" type="checkbox"
                               value=""
                               id="role_{{ $role->name }}"{{ in_array($role->name, old('roles', [])) ? ' checked' : '' }}>
